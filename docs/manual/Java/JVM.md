...
局部变量表: 容量以 Slot(32 位以内)为单位, 变量按一定顺序排列在内, 可以随着方法执行复用
...
随着 JIT 编译器的发展和逃逸分析等技术的成熟, 有时候可以直接在栈上分配对象

2. **方法区**: 存储被虚拟机加载的类型信息、常量、静态变量、即时编译器编译后的代码缓存等数据 (逻辑区域)  
   在 HotSpot 虚拟机中, 永久代原本是方法区的实现。1.7 把字符串常量池和静态变量移出，1.8 将剩余的(类型信息 运行时常量池等)内容全部移到了元空间，直接使用本地内存。  
...
3. **直接内存**: 并不是虚拟机运行时数据区的一部分，也不是《Java 虚拟机规范》中定义的内存区域。但是这部分内存也被频繁地使用  
...
句柄和直接指针之间的差异在于对于对象的访问, 句柄更灵活, 不过要多一次寻址
...
- 非并发 GC(连续 GC):
  ...
  - young GC: eden 区满了触发
  - full GC:
    young GC 的平均晋升大小比剩余的 old gen 空间大, 不进行 young gc, 触发 full gc;  
    或者永久代空间不够;
...
```sh
java -XX:+PrintFlagsFinal -version #all
...
注意: 常量池中是从 1 开始计数的, 所有的没有指向的字面量都会指向常量池中的第 0 个位置
...
2. **连接**:  
   ...
   - 文件格式验证: Class 文件格式规范；这时候会讲字节流转化成方法区的数据格式 - 元数据验证: 对类的元数据信息进行语义校验, 确保符合 Java 语言规范 - 字节码验证: 最复杂的, 通过数据流和控制流分析, 确定程序语义合法 - 符号引用验证: 发生在虚拟机将符号引用转化为直接引用的时候(解析时发生)，确保解析动作能正确执行  
      <br/>
...
类加载器都有范围, 加载范围内没有这个类就无法加载
双亲委派可以有效地避免重复加载的问题.

#### 自定义类加载器

自定义加载器，需要继承抽象类 ClassLoader。
如果我们不想打破双亲委派模型，就重写 findClass() 方法，无法被父类加载器加载的类最终会通过这个方法被加载。
...
1. 编译成 .class 文件
2. 启动 JVM 进程, 类加载
3. JVM 找到主程序入口, 执行 main 方法
4. 方法调用: 根据对象找到类信息的方法表, 获取到方法的字节码地址
