...
进程与轻量级进程的差异: 进程拥有自己独立的地址空间，而轻量级进程没有，只能共享同 1 个轻量级进程组下的地址空间
...
这种场景是计算过 hashcode，位置用来存 hashcode 了，就不能上偏向锁了。
...
偏向的概念: 1 个线程 CAS 加锁后，如果没有其他线程竞争，该线程可以直接获取锁，无需任何 CAS 或互斥操作

当出现线程竞争偏向锁时，偏向锁可能会撤销(撤销后不能再偏向)，撤销需要在 Safe Point 暂停线程 A，检查 A 是否执行完同步代码。

- 如果锁无效，变成无锁状态，可以重偏向
- 如果锁有效，则会升级为轻量级锁

获得偏向锁的线程其栈中也有 Lock Record，作为执行`monitorexit`命令时弹出栈帧的载体，也要作为重入计数，还要在检查点作为线程 A 执行完毕的依据。
...
- 线程的视角：1 个线程发现该对象的锁状态标志为轻量级锁，会在自己的栈中开辟一块空间 "Lock Record"，存放 markword 副本和 owner 指针（指向该对象锁）。
- 作为锁的对象视角：markword 中存放指向线程的指针。

在线程栈中存储主要是为了将 markword 恢复
...
- 如果释放锁 CAS 失败了，也就是说锁标识位是 10，重量级锁，也意味着有其他线程尝试获取过锁，那么除了要释放锁，还要唤醒其他线程。
