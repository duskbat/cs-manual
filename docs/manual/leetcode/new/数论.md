...
> 超出范围的判定, x 的最高位是 1 或 2，反转后最后一位不会超限，所以，只需判定除去最后一位的值在 [int_min/10, int_max/10] 之间即可
...
class Solution {
    public int reverse(int x) {
        ...
        while (x != 0) {
            // x范围：-2147483648 +2147483647
            // 假设数位是满的
            // 负数:最后的余数(原最高位)范围是[-2,-1], 显然大于-8, 不会溢出;
            // 正数:最后的余数(原最高位)范围是[1,2], 显然小于7, 不会溢出;
            // 所以不会因为最后1次溢出，只需判定倒数第2次在 [int_min/10, int_max/10] 之间
...
> 当数字的位数为奇数, 我们可以将较长的半段的末位截掉后, 再对比
...
> 比较烂的一道题  
> 罗马数字前一个比后一个小的话, 表示要减掉前面的，就是说前面一个数字的符号依赖于后一个数字的大小
