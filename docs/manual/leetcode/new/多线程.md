...
class Solution {
    static ReentrantLock lock = new ReentrantLock();
    static Condition cond0;
    static Condition cond1;
    static int cnt;

    public static void main(String[] args) {
        cond0 = lock.newCondition();
        cond1 = lock.newCondition();
        Worker worker0 = new Worker(false);
    ...
    static class Worker extends Thread {
        boolean odd; // worker 带状态

        public Worker(boolean odd) {
            this.odd = odd;
        }
        ...
        public void run() {
            while (cnt < 100) {
                ...
                try {
                    if (odd) {
                        // 偶数则 wait, 并释放锁
                        while (cnt % 2 == 0) cond0.await();
                        System.out.println(this + ":" + cnt++);
                        cond1.signal();
                    } else {
                        while (cnt % 2 == 1) cond1.await();
                        System.out.println(this + ":" + cnt++);
                        cond0.signal();
                    }
