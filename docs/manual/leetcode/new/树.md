...
class Solution {
    ...
    void backtracking(TreeNode node, int diff, List<Integer> list) {
        list.add(node.val);
        ...
        if (node.left != null) backtracking(node.left, diff - node.val, list);
        if (node.right != null) backtracking(node.right, diff - node.val, list);
        list.remove(list.size() - 1);
    }
}
```
